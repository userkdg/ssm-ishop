...
import com.ishop.pojo.TbItemParamItemExample.Criteria;
...
public class ItemServiceImpl implements ItemService {
	...
	@Override
	public IshopResult getItemBaseInfo(long itemId) {
		try {
			//添加缓存逻辑
			//从缓存中取商品信息，商品id对应的信息
	...
	@Override
	public IshopResult getItemDesc(long itemId) {
		//添加缓存
		try {
			//添加缓存逻辑
			//从缓存中取商品信息，商品id对应的信息
			...
			if (!StringUtils.isBlank(json)) {
				...
				return IshopResult.ok(itemDesc);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	@Override
	public IshopResult getItemParam(long itemId) {
		//添加缓存
		try {
			//添加缓存逻辑
			//从缓存中取商品信息，商品id对应的信息
			...
			if (!StringUtils.isBlank(json)) {
				...
				return IshopResult.ok(paramItem);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		return IshopResult.build(400, "无此商品规格");
	}

	@Override
	public IshopResult deleteItem(Long itemId) {
		TbItemExample example = new TbItemExample();
		com.ishop.pojo.TbItemExample.Criteria criteria = example.createCriteria();
