...
public class UserController {
	...
	public Object checkData(@PathVariable String param, @PathVariable Integer type, String callback) {
		...
		IshopResult result = null;
		
		//参数有效性校验
		if (StringUtils.isBlank(param)) {
			result = IshopResult.build(400, "校验内容不能为空");
		}
		if (type == null) {
			result = IshopResult.build(400, "校验内容类型不能为空");
		}
		if (type != 1 && type != 2 && type != 3 ) {
			result = IshopResult.build(400, "校验内容类型错误");
		}
		//校验出错
		if (null != result) {
			if (null != callback) {
		...
		try {
			...
			result = IshopResult.build(500, ExceptionUtil.getStackTrace(e));
		}
	...
	public IshopResult createUser(TbUser user) {
		...
		try {
			IshopResult result = userService.createUser(user);
			return result;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public IshopResult userLogin(String username, String password,
		...
		try {
			...
			IshopResult result = userService.userLogin(username, password, request, response);
			return result;
		} catch (Exception e) {
			e.printStackTrace();
			return IshopResult.build(500, ExceptionUtil.getStackTrace(e));
